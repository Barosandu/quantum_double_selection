...
@resumable function purifier(
    ...
    round = 0
    while true
        pairs_of_bellpairs = findqubitstopurify_doubleselection(network,nodea,nodeb)
        if isnothing(pairs_of_bellpairs)
            @yield timeout(sim, purifier_wait_time)
            continue
        end
        ...
        apply!((rega[pair2qa],rega[pair3qa]), gate)
        apply!((regb[pair2qb],regb[pair3qb]), gate)

        measa = project_traceout!(rega[pair3qa], σˣ)
        measb = project_traceout!(regb[pair3qb], σˣ)

        measa2 = project_traceout!(rega[pair2qa], σᶻ)
        measb2 = project_traceout!(regb[pair2qb], σᶻ)

        if measa!=measb || measa2!=measb2
            traceout!(rega[pair1qa])
            traceout!(regb[pair1qb])
            network[nodea,:enttrackers][pair1qa] = nothing
            network[nodeb,:enttrackers][pair1qb] = nothing
...
function findqubitstopurify_doubleselection(network,nodea,nodeb)
    enttrackers = network[nodea,:enttrackers]
    locksa = network[nodea,:locks]
    locksb = network[nodeb,:locks]
    enttrackers = [(i=i,n...) for (i,n) in enumerate(enttrackers)
